...
document.getElementById('runSim').addEventListener('click', () => {
  ...
  const numRounds = Math.max(1, parseInt(document.getElementById('simRounds').value) || 4);
  ...
  for (let round = 1; round <= numRounds; round++) {
    ...
    if (!fit) {
      ...
      tr.innerHTML = '<td>' + round + '</td><td colspan="2">readings too degenerate to solve, skipped</td>';
